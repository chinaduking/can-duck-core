...
为了进一步扩展可用的实时消息ID的范围，我们利用的标准帧。我们规定所有标准帧全部是实时消息，由此将每个节点可用的实时消息类型扩展到32个。
```cpp
/*
 * 基于11位扩展ID的快实时消息包头
 **/
...
```
### 4.4 分段数据包
不同于CAN Open，为了简化通信解析程序的设计，我们将任何过长的数据包（不论消息还是服务）全部规定为“分段数据包”，并用同一的方式拆包、组包，再进行相应的解析（相当于模拟一个MTU超过8的CAN总线）。为了能更可靠地组包，分段数据包包含一个传输计数和一个状态码，用以标识传输状态。一次可传输的最大数据大小为9*64=576字节 
