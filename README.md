...
```
### 4.4 分段数据包
不同于CAN Open，为了简化通信解析程序的设计，我们将任何过长的数据包（不论消息还是服务）全部规定为“分段数据包”，并用同一的方式拆包、组包，再进行相应的解析（相当于模拟一个MTU超过8的CAN总线）。为了能更可靠地组包，分段数据包
...
struct HeaderSegment{
    ...
    uint32_t data_0  : 8;   /* 起始包：总包数；中间包：数据0；结束包：0 */
};
```
