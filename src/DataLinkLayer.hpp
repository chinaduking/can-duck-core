...
namespace libfcn_v2{
    ...
    public:
        ...
        explicit ByteStreamParser(emlib::Vector<uint8_t>& header_,
                                  int max_buf = DATALINK_MTU + 20);
        ~ByteStreamParser() = default;

        void setHeader(emlib::Vector<uint8_t>& header_);
        int8_t rxParseUpdate(uint8_t recv_byte, FramePtr recv_frame);

    private:
        emlib::Vector<uint8_t> header;
...
//      FramePtr receiving_frame;
    ...
    public:
        ...
        explicit ByteFrameIODevice(LLByteDevice* ll_byte_dev, int frame_buffer_sz=8):
            ...
            emlib::memcpy(header_buf, header.data(), header.size());
        }
